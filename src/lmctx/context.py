...
class _SerializedBlobStore:
    ...
    def __init__(self, entries_by_id: Mapping[str, tuple[BlobReference, bytes]]) -> None:
        """Initialize with preloaded ``(BlobReference, bytes)`` tuples keyed by ID."""
        self._blobs = {str(blob_id): data for blob_id, (_, data) in entries_by_id.items()}
        self._entries = {
            str(blob_id): BlobEntry(
                ref=ref,
                created_at=utc_now(),
                last_accessed_at=None,
            )
            for blob_id, (ref, _) in entries_by_id.items()
        }
    ...
    def list_blobs(
        ...
        *,
        ...
        """List hydrated serialized blobs."""
        entries = tuple(
    ...
    def prune_blobs(
        ...
        *,
        ...
        """Prune hydrated serialized blobs."""
...
def _deserialize_blob_store(value: object) -> BlobStore:
    ...
    return _SerializedBlobStore(entries_by_id)
