"""Anthropic Messages API: build requests with lmctx, call with the Anthropic SDK.
...
# Anthropic's latest active model as of 2026-02-11.
...
def to_dict(value: object) -> dict[str, object]:
    mapped = _as_str_object_dict(value)
...
response = client.messages.create(**plan.request)
...
japanese_response = client.messages.create(**japanese_plan.request)
...
schema_response = client.messages.create(**schema_plan.request)
...
for turn in range(4):
    ...
    response2 = client.messages.create(**plan2.request)
...
print(f"  Request tools: {server_tool_plan.request.get('tools')}")
server_tool_response = client.messages.create(**server_tool_plan.request)
...
if server_last:
    text_chunks = [part.text for part in server_last.parts if part.type == "text" and part.text]
    if text_chunks:
        print(f"  Web search answer: {''.join(text_chunks)}")
...
thinking_response = client.messages.create(**thinking_plan.request)
...
print("\n=== Streaming run ===")
with client.messages.stream(**stream_plan.request) as stream:
    for text in stream.text_stream:
        print(text, end="", flush=True)
    stream_response = stream.get_final_message()
print()
...
image_response = client.messages.create(**image_plan.request)
...
# ── 7. File upload run (PDF, beta APIs) ────────────────────────────

print("\n=== File upload run (PDF, beta APIs) ===")
with PDF_PATH.open("rb") as pdf_file:
    uploaded = client.beta.files.upload(file=pdf_file, betas=[FILES_BETA])
...
upload_spec = RunSpec(
    ...
    endpoint="messages.create",
    model=MODEL,
    extra_body={"betas": [FILES_BETA, PDFS_BETA]},
)
upload_plan = adapter.plan(ctx6, upload_spec)
beta_response = client.beta.messages.create(**upload_plan.request)
actual_model = verify_called_model(upload_spec.model, getattr(beta_response, "model", None))
print(f"  Model verified: requested={upload_spec.model}, actual={actual_model}")
print(f"  Uploaded file_id: {file_id}")

ctx6 = adapter.ingest(ctx6, beta_response, spec=upload_spec)
...
if not printed_pdf_answer:
    beta_data = to_dict(beta_response)
    content = beta_data.get("content")
    if isinstance(content, list):
        for block in content:
            block_data = _as_str_object_dict(block)
            if block_data is not None and block_data.get("type") == "text":
                text = block_data.get("text")
                if isinstance(text, str) and text:
                    print(f"  PDF answer: {text}")
                    break
